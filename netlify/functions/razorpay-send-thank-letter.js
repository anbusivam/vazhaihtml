...
// Includes the donor's additionalEmails (from their profile) in the CC field,
// and attaches the PDF donation receipt generated by razorpay-receipt-pdf.js.
const { getStore, ADMIN_EMAILS } = require('./auth-store');
const fs = require('fs');
const path = require('path');
const { getUserProfile, generateReceiptPDFForPayment } = require('./razorpay-receipt-pdf');
...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    // ── Look up the user's saved profile for [user-name], [user-tamilname], and additionalEmails ──
    let userName = '';
    let userTamilName = '';
    ...
    if (donorEmail) {
      ...
      if (userProfile) {
        ...
        additionalEmails = Array.isArray(userProfile.additionalEmails) ? userProfile.additionalEmails : [];
      }
    }
    ...
    // ── Build CC list: always include vazhai.connect@gmail.com, plus the donor's additionalEmails ──
    const ccList = ['vazhai.connect@gmail.com'];
    for (const email of additionalEmails) {
      const normalized = email.trim().toLowerCase();
      if (normalized && !ccList.includes(normalized) && normalized !== donorEmail.toLowerCase().trim()) {
        ccList.push(normalized);
      }
    }

    // ── Generate the PDF receipt to attach ──
    let pdfBuffer = null;
    try {
      const donorProfile = await getUserProfile(store, donorEmail.toLowerCase().trim());
      const receiptUserName = donorProfile.name || payment.donorName || donorEmail.split('@')[0] || 'Donor';
      const receiptUserAddress = donorProfile.address || payment.donorAddress || '';
      const receiptUserPan = donorProfile.pan || payment.donorPan || '';

      pdfBuffer = await generateReceiptPDFForPayment(store, {
        userName: receiptUserName,
        userEmail: donorEmail,
        userAddress: receiptUserAddress,
        userPan: receiptUserPan,
        payment,
      });
      console.log('[razorpay-send-thank-letter] PDF receipt generated for payment', paymentId, '(', pdfBuffer.length, 'bytes )');
    } catch (pdfErr) {
      console.error('[razorpay-send-thank-letter] Failed to generate PDF receipt:', pdfErr.message);
      // Non-fatal: send the email without the attachment
    }
    ...
    // Build the email payload. If the PDF was generated, attach it.
    const emailPayload = {
      from: fromEmail,
    ...
    if (pdfBuffer) {
      emailPayload.attachments = [
        {
          filename: `Vazhai_Receipt_${paymentId}.pdf`,
